...
void grid(Monitor *m)
{
	...
	ch = (m->wh - 2 * oh) / (rows ? rows : 1) - ih * ((rows ? rows : 1) - 1);
	cw = (m->ww - 2 * ov) / (cols ? cols : 1) - iv * ((cols ? cols : 1) - 1);

	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next))
	{
		cx = m->wx + (i / rows) * cw + ov + iv * (i / rows);
		cy = m->wy + (i % rows) * ch + oh + ih * (i % rows);
		/* adjust height/width of last row/column's windows */
		ah = ((i + 1) % rows == 0) ? m->wh - ch * rows - oh * 2 - ih * 2 : 0;
		aw = (i >= rows * (cols - 1)) ? m->ww - cw * cols - ov * 2 - iv * 2 : 0;
